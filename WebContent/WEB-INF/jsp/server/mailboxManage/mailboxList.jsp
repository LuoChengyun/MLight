...
<body>
		...
		<blockquote class="layui-elem-quote news_search">
			<div class="layui-form-item">
				<div class="layui-inline pane-select">
					<label class="layui-form-label">信件编号</label>
					<div class="layui-input-block ">
						<input type="text" id="mailboxId" name="mailboxId"
							lay-verify="required|number" placeholder="请输入信件编号"
	...
	<script type="text/html" id="toolBar">
		<a class="layui-btn layui-btn-mini" lay-event="updateMailbox" >编辑</a>
		<a class="layui-btn layui-btn-mini layui-btn-danger" lay-event="deleteMailbox" >删除</a>
	</script>
	...
	<script>
		...
		function doSearch() {
			...
			var title = $("#title").val();
			getlist(mailboxId, title);
		}

		function getlist(mailboxId, title) {
			tableIns = table.render({
				...
				url : '${ctx}/server/mailbox/getMailboxList.action',
				cols : [ [ {
					field : 'id',
					title : '编号',
					width : 80,
		...
		table.on('tool(forum)', function(obj) {
			var data = obj.data;
			if (obj.event === 'updateMailbox') {
				var index = layui.layer.open({
					title : "查看信件",
					type : 2,
					content : "${ctx}/server/mailbox/goUpdateMailbox.action?id="
							+ data.id,
			...
			}else if (obj.event === 'deleteMailbox') {
				layer.confirm('确定删除该条信件吗?', function(index){
			        //向服务端发送删除指令
			        $.ajax({
						type : "post",
						url : "${ctx}/server/mailbox/deleteMailbox.action",
						dataType : "json",
						data : {
							"id" : data.id,
						},
